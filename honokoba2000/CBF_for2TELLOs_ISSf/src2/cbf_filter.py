...
class CBFParams:
    def __init__(self, K1=0.009, K2=0.009, alpha1=1.0, alpha2=1.0, alpha3=1.0, 
                 ...
                 use_issf_constraint: bool = False,
                 issf_gain: float = 4.0,
                 ips0: float = 0.1,
        ...
        self.use_issf_constraint = use_issf_constraint
        self.issf_gain = issf_gain
        self.ips0 = ips0
...
def issf_halfspace_coeffs(x: float, y: float, x_dot: float, y_dot: float,
                          x_o: float, y_o: float, p: CBFParams) -> Tuple[np.ndarray, float]:
    """
    ISSf制約の半空間係数を計算
    a = [2*K1*(x-x_o), 2*K2*(y-y_o)]
    ...
    dist_sq = dx * dx + dy * dy
    exp_arg = p.lambda_param * (dist_sq - p.Delta ** 2)
    if exp_arg > 80.0:
        exp_arg = 80.0

    issf_term = -(p.issf_gain * dist_sq) / (p.ips0 * math.exp(exp_arg))
    b = (
        2.0 * (x_dot ** 2) + 2.0 * (p.alpha2 + p.alpha3) * dx * x_dot
        + 2.0 * (y_dot ** 2) + 2.0 * (p.alpha2 + p.alpha3) * dy * y_dot
        + p.alpha1 * p.alpha2 * (dx * dx + dy * dy - p.Delta * p.Delta)
...
def enforce_cbf(u_nom: Tuple[float, float],
    ...
    mode_name = "ISSf" if getattr(params, 'use_issf_constraint', False) else "CBF"

    # h1: 障害物回避制約
    if getattr(params, 'use_issf_constraint', False):
        a1, b1 = issf_halfspace_coeffs(x, y, x_dot, y_dot, x_o, y_o, params)
    else:
    ...
    if all_satisfied:
        ...
        print(f"🜢 {mode_name}: すべての制約満足 h1={h1_value:.3f}, h2={h2_value:.3f}, h3={h3_value:.3f}")
        return u, info
    else:
        print(f"🔴 {mode_name}: 制約違反 h1={h1_value:.3f}(ok={info['h1_satisfied']}), h2={h2_value:.3f}(ok={info['h2_satisfied']}), h3={h3_value:.3f}(ok={info['h3_satisfied']})")
    ...
    if all_satisfied_after:
        info['projected'] = True
        print(f"🟡 {mode_name}: 逐次射影成功 h1={h1_value:.3f}, h2={h2_value:.3f}, h3={h3_value:.3f}")
        return u, info
    else:
        # 完全には満たせなかった場合（保守的）
        info['fell_back'] = True
        print(f"🟠 {mode_name}: 逐次射影後も制約違反 h1={h1_value:.3f}(ok={info['h1_satisfied']}), h2={h2_value:.3f}(ok={info['h2_satisfied']}), h3={h3_value:.3f}(ok={info['h3_satisfied']})")
        return u, info
