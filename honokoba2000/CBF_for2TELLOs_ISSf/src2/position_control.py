...
class PositionController:
    ...
    def __init__(self):
        ...
        self.cbf_params = CBFParams(
            ...
            Delta=0.8366,  # 障害物回避の安全半径 [m]
            v_ref=0.01,  # 速度差制約の参照値（現在未使用）
            p_ref=1.0,  # 機体間距離の下限 [m] (test.mdより更新)
            enable_velocity_constraints=True,  # 2式目（機体間距離CBF）をON
            use_issf_constraint=True,
            issf_gain=4.0,
            ips0=4,
            lambda_param=3,
        )
    ...
    def calculate_control(self, current_position, current_yaw=None, quaternion=None):
        ...
        mode_name = "ISSf" if getattr(self.cbf_params, 'use_issf_constraint', False) else "CBF"
        print(f"🔍 {mode_name}処理チェック: cbf_enabled={self.cbf_enabled}")
        logging.info(f"{mode_name}処理チェック: cbf_enabled={self.cbf_enabled}")
        if self.cbf_enabled:
            # MOCAP差分から速度推定（前処理なし）
            now = time.time()
            ...
            issf_term = 0.0
            if getattr(self.cbf_params, 'use_issf_constraint', False):
                exp_arg = self.cbf_params.lambda_param * (dist_sq - Delta**2)
                if exp_arg > 80.0:
                    exp_arg = 80.0
                issf_term = -(self.cbf_params.issf_gain * dist_sq / self.cbf_params.ips0) * math.exp(exp_arg)
                HOh_x = HOh_x + issf_term
            ...
            print(f"🚀 {mode_name}実行: rc_nom=[{rc_nom_x:.1f},{rc_nom_z:.1f}] -> rc_safe=[{rc_safe_x:.1f},{rc_safe_z:.1f}], diff_norm={rc_diff_norm:.3f}")
            print(f"📊 {mode_name}数学値: h(x)={h_x:.3f}, HOh(x)={HOh_x:.3f}")
            print(f"🔍 {mode_name}詳細: 位置=({x_test:.2f},{y_test:.2f}), 障害物=({x_o},{y_o}), 距離²={dist_sq:.3f}, Δ²={Delta**2:.3f}")
            print(f"🔍 {mode_name}分解: velocity={velocity_term:.3f}, coupling={coupling_term:.3f}, control={control_term:.3f}")
            print(f"🔍 {mode_name}分解: position={position_term:.3f}, barrier={barrier_term:.3f}")
            if getattr(self.cbf_params, 'use_issf_constraint', False):
                print(f"🔍 {mode_name}分解: issf={issf_term:.3f}")
            print(f"🔍 {mode_name}状態: {'制約違反' if HOh_x < 0 else '制約満足'}, QP状態={info.get('qp_status', 'unknown')}")
            logging.info(f"{mode_name}実行: rc_nom=[{rc_nom_x:.1f},{rc_nom_z:.1f}] -> rc_safe=[{rc_safe_x:.1f},{rc_safe_z:.1f}], diff_norm={rc_diff_norm:.3f}")
            logging.info(f"{mode_name}制約: HOh_x={HOh_x:.3f}, h(x)={h_x:.3f}")
