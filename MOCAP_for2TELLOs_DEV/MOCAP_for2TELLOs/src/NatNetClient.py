...
# socket.timeoutの正しい参照を確保
timeout_error = socket.timeout

# ロギング設定 - 完全無効化
# キーボード入力応答性のため、全てのログ出力を停止
log_format = '%(asctime)s - %(levelname)s - [NATNET] %(message)s'
logging.getLogger().setLevel(logging.CRITICAL)  # rootロガーも強制的に無効化
if not logging.getLogger().handlers:
    logging.basicConfig(level=logging.CRITICAL, format=log_format)

def trace(*args):
    # デバッグ出力を一時的に無効化（パフォーマンス向上のため）
    pass
    # 元のデバッグコード（必要に応じて復活させる）
    # if not hasattr(trace, 'stop_flag') or not trace.stop_flag:
    #     logging.debug("".join(map(str, args)))

# スレッド終了フラグの初期化
trace.stop_flag = False
...
class NatNetClient:
    ...
    def __createDataSocket(self, port):
        logging.info(f"__createDataSocket: ポート {port} のデータソケットを作成します")
        logging.info(f"__createDataSocket: マルチキャストアドレス={self.multicastAddress}, ローカルアドレス={self.localIPAddress}")
        
        try:
            # ソケットの作成
            result = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
            logging.info("__createDataSocket: ソケットを作成しました")

            # ソケットオプションの設定
            result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            logging.info("__createDataSocket: SO_REUSEADDRオプションを設定しました")
            
            # ブロードキャスト受信を有効化
            result.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
            logging.info("__createDataSocket: SO_BROADCASTオプションを設定しました")
            
            # ソケットのバインド - 重要: 空文字列を使用して任意のアドレスからのパケットを受信
            result.bind(('', port))  # 空文字列を使用することが重要
            logging.info(f"__createDataSocket: ソケットをポート {port} にバインドしました (任意のアドレスからの受信)")
            ...
            try:
                ...
                logging.info("__createDataSocket: IP_ADD_MEMBERSHIPオプションを設定しました")
                
                # マルチキャストTTLの設定
                result.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, 32) 
                logging.info("__createDataSocket: IP_MULTICAST_TTLを32に設定しました")
                
                # マルチキャストループバックを有効化
                result.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_LOOP, 1)
                logging.info("__createDataSocket: IP_MULTICAST_LOOPを有効化しました")
                
                # タイムアウトの設定（デバッグ用）
                # タイムアウトは長めに設定して、データ受信を待つ時間を確保
                result.settimeout(30.0)  # 30秒のタイムアウト
                logging.info("__createDataSocket: ソケットタイムアウトを30秒に設定しました")
        ...
        try:
            ...
            for i in range(markerSetCount):
                ...
                for j in range(markerCount):
                    _pos = struct.unpack('fff', data[offset:offset+12])
                    offset += 12
                    #trace("\tMarker", j, ":", _pos[0], ",", _pos[1], ",", _pos[2])
            ...
            for i in range(unlabeledMarkersCount):
                _pos = struct.unpack('fff', data[offset:offset+12])
                offset += 12
                #trace("\tMarker", i, ":", _pos[0], ",", _pos[1], ",", _pos[2])
    ...
    def __dataThreadFunction(self, socket):
        logging.info(f"__dataThreadFunction: データ受信スレッドを開始しました")
        logging.info(f"__dataThreadFunction: マルチキャストアドレス={self.multicastAddress}, ローカルアドレス={self.localIPAddress}")
        ...
        try:
            logging.info("__dataThreadFunction: データを待機中...")
            
            while True:
                # スレッド終了フラグをチェック
                if hasattr(trace, 'stop_flag') and trace.stop_flag:
                    logging.info("__dataThreadFunction: 終了フラグが設定されました。スレッドを終了します。")
                    break
                    
                try:
                    # データ受信を試行
                    data, addr = socket.recvfrom(SOCKET_BUFFER_SIZE)  # 定義されたバッファサイズを使用
                    ...
                    if e.errno == 9:  # Bad file descriptor
                        ...
                        logging.error(f"__dataThreadFunction: ソケットエラーが発生しました: {e}", exc_info=True)
                except Exception as e:
                    logging.error(f"__dataThreadFunction: メインループでエラーが発生しました: {e}", exc_info=True)
                    if wait_count % 5 == 0:
                        logging.warning(f"__dataThreadFunction: ソケットタイムアウトが発生しました ({wait_count}回目)")
                    ...
                    logging.error(f"__dataThreadFunction: データ受信中にエラーが発生しました: {e}", exc_info=True)
                    import traceback
                    traceback.print_exc()
    ...
    def __processMessage(self, data):
        try:
            ...
            logging.error(f"__processMessage: エラーが発生しました: {e}", exc_info=True)
    ...
    def run(self):
        ...
        try:
            ...
            logging.error(f"NatNetClient: データソケットの作成中にエラーが発生しました: {e}", exc_info=True)
            return False
        ...
        try:
            ...
            logging.error(f"NatNetClient: コマンドソケットの作成中にエラーが発生しました: {e}", exc_info=True)
            return False
    ...
    def shutdown(self):
        ...
        try:
            ...
            logging.error(f"NatNetClient: シャットダウン中にエラーが発生しました: {e}", exc_info=True)
