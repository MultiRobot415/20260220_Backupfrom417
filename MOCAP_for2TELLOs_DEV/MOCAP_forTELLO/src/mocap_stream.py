...
current_rot = {"x": 0.0, "y": 0.0, "z": 0.0, "w": 1.0}
current_error = 0.0

# データバッファの設定
MAX_BUFFER_SIZE = 100  # 最大バッファサイズ（保持するフレーム数）
position_buffer = deque(maxlen=MAX_BUFFER_SIZE)  # 位置データバッファ
rotation_buffer = deque(maxlen=MAX_BUFFER_SIZE)  # 回転データバッファ
error_buffer = deque(maxlen=MAX_BUFFER_SIZE)     # エラーデータバッファ
...
# メモリ管理の設定
GC_INTERVAL = 100  # 何パケットごとにガベージコレクションを実行するか
...
def get_connection_status():
    """
    接続状態を取得する
    :return: 接続状態の辞書 {"connected": bool, "last_packet_time": float, "packets": int}
    """
    current_time = time.time()
    connected = (last_packet_time is not None and current_time - last_packet_time < 5)
