...
class PositionController:
    ...
    def __init__(self):
        ...
        デフォルトのゲイン値と制限値を設定
        """
        # P制御のゲイン係数（x, y, z, yaw）- 安定性重視に調整
        self.gain_x = 1.0  # 前後方向のゲイン (安定性重視: 1.6→1.0)
        self.gain_y = 1.0  # 上下方向のゲイン (安定性重視: 1.6→1.0)
        self.gain_z = 1.0  # 左右方向のゲイン (安定性重視: 1.6→1.0)
        self.gain_yaw = 0.2  # 旋回方向のゲイン (精度向上: 0.15→0.2)
        
        # 速度の制限値 - ハイゲイン化に伴い安全性を考慮して調整
        self.max_speed_x = 30  # 前後方向の最大速度 (安全性向上: 20→18)
        self.max_speed_y = 30  # 上下方向の最大速度 (安全性向上: 20→18)
        self.max_speed_z = 30  # 左右方向の最大速度 (安全性向上: 20→18)
        self.max_speed_yaw = 25  # 旋回方向の最大速度 (応答性向上: 20→25)
        ...
        self.k1 = 5.0  # 位置ゲイン（定常偏差改善: 2.0→3.0）
        self.k2 = 2.0  # 速度ゲイン（安定性向上: 2.0→1.0）
    ...
    def _update_formation_offset(self):
        ...
        if self.is_leader:
            ...
            # フォロワーのオフセット（ドローンIDに基づく） - Z軸方向に配置
            if self.drone_id == 0:  # 1号機がフォロワーの場合
                self.formation_offset = [0.0, 0.0, 0.0]  # 基準位置
            elif self.drone_id == 1:  # 2号機がフォロワーの場合
                self.formation_offset = [0.5, 0.0, 0.5]  # Z軸方向に+1mオフセット
            elif self.drone_id == 2:  # 3号機がフォロワーの場合
                self.formation_offset = [0.5, 0.0, -0.5]  # Z軸方向に-1mオフセット
