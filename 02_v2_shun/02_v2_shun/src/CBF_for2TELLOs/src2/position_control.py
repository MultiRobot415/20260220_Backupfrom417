...
class PositionController:
    ...
    def __init__(self):
        ...
        self.k1 = 5.0  # 位置ゲイン（定常偏差改善: 2.0→3.0→5.0）
        self.k2 = 4.0  # 速度ゲイン（フォーメーション維持: 2.0→3.0→4.0）
        ...
        self.cbf_params = CBFParams(
            ...
            Delta=0.0,  # test.mdより更新: Δ=0 (障害物回避制約無効化)
            v_ref=0.0005,  # test.mdより更新: v_ref=0.0005
            enable_velocity_constraints=True  # 速度差制約ON
        )
        # 障害物はtest座標系で設定（x_test, y_test）
        self.cbf_obstacle_test = (-0.5, -0.2)  # ユーザー指定: test座標系(xo,yo)=(-0.5,-0.2)
        # 速度算出用（MOCAP差分）
    ...
    def calculate_control(self, current_position, current_yaw=None, quaternion=None):
        ...
        if self.cbf_enabled:
            ...
            if self.cbf_params.enable_velocity_constraints and len(self.other_drone_velocities) > 0:
                # 他のドローンのIDを取得（自分以外）
                other_ids = [id for id in self.other_drone_velocities.keys() if id != self.drone_id]
                if other_ids:
                    other_id = other_ids[0]  # 2台システムなので最初の1台
                    other_vel = self.other_drone_velocities[other_id]
                    # proj座標系 [vx, vy, vz] -> test座標系 [xdot_test, ydot_test]
                    # x_test<-Z_proj, y_test<-X_proj
                    other_velocity_test = (other_vel[2], other_vel[0])

            # 名目入力（test座標系）: u_x(test)=rc_pitch(control_x), u_y(test)=rc_roll(control_z)
            ...
            rc_nom_x, rc_nom_z = float(control_x), float(control_z)  # 名目入力（修正前）
            rc_safe_x, rc_safe_z = float(u_safe[0]), float(u_safe[1])  # CBF適用後
            ...
            # 結果をproj/Tello入力へ反映（rc_fb=proj X= u_x, rc_lr=proj Z= u_y）
            control_x = int(self._clamp(float(u_safe[0]), -self.max_speed_x, self.max_speed_x))
            control_z = int(self._clamp(float(u_safe[1]), -self.max_speed_z, self.max_speed_z))
