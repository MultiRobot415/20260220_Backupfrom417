...
def init():
    ...
    # ウィンドウを作成
    _win = pygame.display.set_mode((800, 500))  # 高さを増やして新しいテキスト行を追加
    pygame.display.set_caption("Tello Control - Keyboard Input Window (Click Here)")
    _win.fill((200, 200, 200))  # Original gray background
    
    # テキスト描画
    try:
        font = pygame.font.Font(None, 32)  # Larger font size
        text1 = font.render("CLICK THIS WINDOW FOR KEYBOARD FOCUS", True, (0, 0, 255))
        text2 = font.render("T key: SLAF Mode | M key: Manual Mode", True, (255, 0, 0))
        ...
        text5 = font.render("Keyboard Status: OK if window is active", True, (255, 0, 0))
        text6 = font.render("YOU MUST CLICK THIS WINDOW", True, (128, 0, 128))
        text7 = font.render("H: H-Mode Start (Trajectory Gen) | J: H-Mode Stop", True, (0, 128, 0))
        text8 = font.render("O: Occlusion ON (Follower4) | P: Occlusion OFF", True, (255, 128, 0))
        
        _win.blit(text1, (20, 40))
        _win.blit(text6, (20, 80))
        _win.blit(text2, (20, 150))
        _win.blit(text3, (20, 200))
        _win.blit(text4, (20, 250))
        _win.blit(text7, (20, 300))
        _win.blit(text8, (20, 350))  # オクルージョンキーの説明
        _win.blit(text5, (20, 400))  # 位置を調整
    ...
    while _is_running and local_running:
        try:
            ...
            try:
                ...
                # 定期的にウィンドウのアクティブ状態をチェック（頑度を減らす）
                if time.time() % 60 < 0.1:  # 60秒に一度程度チェック
                    active = pygame.display.get_active()
                    print(f"Pygameウィンドウアクティブ状態: {active}")
                
                # キー入力のデバッグメッセージを削除（パフォーマンス向上）
                # if current_pressed:
    ...
    while _is_running:
        try:
            ...
            if _win and pygame.get_init():
                ...
                # 何か変化を加えることでウィンドウが反応していることを示す
                # 10秒に一度背景色を変更
                # No color changing animation, keep a consistent gray background
                ...
                try:
                    ...
                    text1 = font.render("CLICK THIS WINDOW FOR KEYBOARD FOCUS", True, (0, 0, 255))
                    text2 = font.render("T key: Auto Hovering Mode | M key: Manual Mode", True, (255, 0, 0))
                    text3 = font.render("Q: Takeoff | E: Land | W/S: Up/Down | A/D: Yaw", True, (0, 0, 0))
                    text4 = font.render("Arrows: Movement | SPACE: Exit | ESC: Emergency", True, (0, 0, 0))
                    text5 = font.render("Keyboard Status: " + ("OK" if pygame.display.get_active() else "WINDOW NOT ACTIVE"), True, (255, 0, 0))
                    text6 = font.render("YOU MUST CLICK THIS WINDOW", True, (128, 0, 128))
